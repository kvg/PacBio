...
Unamplified perfect var genes: `r sum(subset(vars, IS_ALIGNED_U == TRUE)$IS_PERFECT_U)`/`r nrow(vars)`.

Amplified perfect var genes: `r sum(subset(vars, IS_ALIGNED_A == TRUE)$IS_PERFECT_A)`/`r nrow(vars)`.
...
kable(exons.amp.df[unalignedExons,]);
```

Exon 1 aligned: `r sum(exons.amp.df[unalignedExon1Names,]$IS_ALIGNED)`.

Exon 2 aligned: `r sum(exons.amp.df[unalignedExon2Names,]$IS_ALIGNED)`.
