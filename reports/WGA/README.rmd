...
legend("bottomright", c("Amplified", "Unamplified"), col=c(color.amp, color.unamp), lwd=2, bty="n");
```

```{r asmStats, echo=FALSE, results='asis'}
asmStats = read.table(paste(results.dir, "assembly.stats", sep="/"), header=TRUE);

asmStats[which(asmStats$id == "amplified"),   "ng50"] = ng50(f.amp,   f.ref);
asmStats[which(asmStats$id == "unamplified"), "ng50"] = ng50(f.unamp, f.ref);

kable(asmStats[,c("id", "numContigs", "minLength", "maxLength", "ng50", "totalSequence")]);
```
