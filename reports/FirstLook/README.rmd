...
We first examined read count and read length properties, as shown in the table above.  We have labeled data from each SMRT cell with a simple identifier, pb0 .. pb7 (the full identifier is a bit long and cumbersome to use). On average, each SMRT cell yields `r mean(subset(ls, key != "total")$numReads)` reads, typically `r mean(subset(ls, key != "total")$meanLength` bases long.  Most reads (~ `r mean(subset(ls, key != "total")$pctAligned)`) align to the reference genome.
...
cov.pb.mean = mean(subset(cov.pb, chrom == "Pf3D7_08_v3")$cov)
cov.pb.sd = sd(subset(cov.pb, chrom == "Pf3D7_08_v3")$cov)
cov.pb.median = median(subset(cov.pb, chrom == "Pf3D7_08_v3")$cov)

cov.il.mean = mean(subset(cov.il, chrom == "Pf3D7_08_v3")$cov)
cov.il.sd = sd(subset(cov.il, chrom == "Pf3D7_08_v3")$cov)
cov.il.median = median(subset(cov.il, chrom == "Pf3D7_08_v3")$cov)

cov.table = rbind(PacBio = c(cov.pb.mean, cov.pb.sd, cov.pb.median), Illumina = c(cov.il.mean, cov.il.sd, cov.il.median));
colnames(cov.table) = c("mean", "sd", "median");
