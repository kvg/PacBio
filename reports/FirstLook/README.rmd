...
for (chr in chroms) {
    ...
    pb.median = median(pb.covs$cov);
    il.median = median(il.covs$cov);

    points(pb.mids, pos + 0.1 +  0.3*(pb.covs$cov / pb.median), type="l", col=color.pb, lwd=0.7);
    points(il.mids, pos - 0.1 + -0.3*(il.covs$cov / il.median), type="l", col=color.il, lwd=0.7);
}
```

```{r runningCorrelationOfCoverage, echo=FALSE}
