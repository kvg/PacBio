...
for (chr in chroms) {
    ...
    il.mincov = c();
    for (i in interval) {
        pb.mincov = c(pb.mincov, min(pb.covs$cov[i:(i+5000)]));
        il.mincov = c(il.mincov, min(il.covs$cov[i:(i+5000)]));
    }
    ...
    points(pb.start[interval], pos + 0.1 +  0.3*(pb.mincov / pb.max), type="l", col=color.pb, lwd=0.5);
    points(il.start[interval], pos - 0.1 + -0.3*(il.mincov / il.max), type="l", col=color.il, lwd=0.5);
}
```
