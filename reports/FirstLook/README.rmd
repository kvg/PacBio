...
for (chr in chroms) {
    ...
    window = 2000;
    interval = seq(1, length(pb.start), by=2000);

    pb.mincov = c();
    il.mincov = c();
    for (i in interval) {
        pb.mincov = c(pb.mincov, min(pb.covs$cov[i:(i+2000)]));
        il.mincov = c(il.mincov, min(il.covs$cov[i:(i+2000)]));
    }
    ...
    points(pb.start[interval], pos + 0.15 +  0.3*(pb.mincov / max(pb.mincov)), type="l", col=color.pb, lwd=0.5);
    points(il.start[interval], pos - 0.15 + -0.3*(il.mincov / max(il.mincov)), type="l", col=color.il, lwd=0.5);
}
```
