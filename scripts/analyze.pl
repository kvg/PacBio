...
my %fqs = (
    ...
    'amplified'   => 'data/LID50115/LID50115_filtered_subreads.fastq'
);
...
foreach my $key (keys(%fqs)) {
    ...
    my $chimeraTable = "$resultsDir/$key/$key.chimeras.k21.txt";
    my $chimeraTableCmd = "$indiana16 FindChimericLongReads -t $chrKmersK21 -l $fq -o $chimeraTable";
    $dm->addRule($chimeraTable, $chrKmersK21, $chimeraTableCmd);

    my $sam = "$resultsDir/$key/$key.sam";
    my $samCmd = "$bwa mem -x pacbio -t 10 $ref $fq > $sam";
